...
var getTotalCount = function(collection, query={}){
    var  q = Object.assign({"name":{$ne:"ids"}},query)
    return collection.count(q)
};

module.exports.getBlogs = function(page=1, rows=10, q={}) {
    var blogs = db.get('blogs')
    var total
    getTotalCount(blogs, q).then((count) => {
      total = count
    })
    var query = Object.assign({"name":{$ne:"ids"}},q)
...
module.exports.getBlog = function(id){
  var collection = db.get('blogs')
  return collection.findOneAndUpdate({'id': parseInt(id),"name":{$ne:"ids"}},{$inc:{'watch_count':1}}).then((docs) => {
    return docs
  })
}
