...
module.exports.getBlogsByTag = function(bid) {
    ...
    tags.find({"name":{$ne:"ids"},"id": parseInt(bid)},{'name': 1}).then(function(docs){
      tagName = docs.name
      return blogs.find({"name":{$ne:"ids"},"tag":tagName}).then(function(docs){
        return docs
      })
    })
}
module.exports.getBlogsByCatagory = function(bid) {
    ...
    catagorys.find({"name":{$ne:"ids"},"id": parseInt(bid)},{'name': 1}).then(function(docs){
      catagoryName = docs.name
      return blogs.find({"name":{$ne:"ids"},"catagory":catagoryName}).then(function(docs){
        return docs
      })
    })
}
